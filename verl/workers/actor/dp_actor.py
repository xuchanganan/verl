            ...
            if self.use_remove_padding:
                ...
                if self.use_ulysses_sp:
                    ...
                    if get_logits:
                        ...
                        top_k_indices_rmpad = gather_outputs_and_unpad(
                            top_k_indices_rmpad,
                            gather_dim=0,
                            unpad_dim=0,
                            padding_size=pad_size,
                ...
                if get_logits:
                    ...
                    top_k_indices = pad_input(
                        hidden_states=top_k_indices_rmpad.unsqueeze(-1),
                        indices=indices,
                        batch=batch_size,
                        seqlen=seqlen,
                    )
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    teacher_log_prob = model_inputs.get("teacher_log_prob", None)
                    if teacher_log_prob is not None:
                        reverse_kl = kl_penalty(log_prob, teacher_log_prob, kl_penalty='k3')
                        advantages = -1 * reverse_kl
